...
def segment_and_sort_frames(frames_dir, output_dir):
    ...
    for frame_file in frames_with_masks:
        frame_path = os.path.join(frames_dir, frame_file)
        shutil.copy(frame_path, os.path.join(cells_dir, frame_file))
    ...
    for frame_file in frames_without_masks:
        frame_path = os.path.join(frames_dir, frame_file)
        shutil.copy(frame_path, os.path.join(background_dir, frame_file))

    print(f"Done! {len(frames_with_masks)} with masks, {len(frames_without_masks)} without masks.")

    # After copying files, save logs to text file
...
# ========== USAGE ==========
# Set your frame folder and output root directory manually here
frames_path = r"D:\JetBrains\PycharmProjects\OAH_shlomi\pythonProject1\HTB5-170122_frames"
output_base = r"D:\JetBrains\PycharmProjects\OAH_shlomi\pythonProject1"

segment_and_sort_frames(frames_path, output_base)